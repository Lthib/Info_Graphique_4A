//Ajouts des keyframes du FOV : Maxime Garibaldi/Loris Thibaud
...
void Camera::do_animate(float time)
{
    ...
    //mon ajout pour gérer les keyframes de focale.
    if (!m_fovKeyframes.empty())//Si on a rajouté des keyframes
    {
    m_fov = m_fovKeyframes.interpolate(time);// Cela trouve la focale en un temps time
    setFov(m_fov);//Puis la setup.
    }//Et on remercie Alfred Hitchcock pour l'effet Vertigo qu'on va pouvoir faire
    //Mais on ne le remercie pas pour sa façon de diriger ses actrices
    //Aucune femme ne devrait être objectifié et sacrifié pour l'oeuvre d'un homme
    // https://www.bbc.com/news/entertainment-arts-37821400
}

void Camera::addFovKeyframe(float fov, float time)
{
    m_fovKeyframes.add(fov, time);//rajoute une keyframe de temps, un peu comme les keyframes de position classiques.
}
