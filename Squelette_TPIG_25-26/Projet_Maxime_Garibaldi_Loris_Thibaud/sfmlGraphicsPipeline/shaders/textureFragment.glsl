...
vec3 papier(vec3 color, vec3 position)
{
    ...
    float n = fract(sin(dot(position.xy * grain, vec2(12.9898, 78.233))) * 43758.5453);//on se base sur la position

    float paper = mix(1.0 - intensity, 1.0 + intensity, n);//pour controler l'amplitude
...
void main()
{
    ...
    tmpColor = papier(tmpColor, surfel_to_camera);
    tmpColor = clamp(tmpColor, 0.05, 0.95);//on empèche les couleurs d'être tout à fait noires ou tout à fait blanches
