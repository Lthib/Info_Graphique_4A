...
vec3 remap(vec3 color)
{
    return color * 0.9 + 0.05;//j'ai rajouté ça pour avoir des gris au lieu des noirs
}

vec3 contourNoir(vec3 color, vec3 N, vec3 V)
{
    float ndotv = max(dot(N, V), 0.0); //N c'est la normale à l'objet, V c'est le vecteur direction de la caméra.
    
    //Si c'est plus petit que 0.6, c'est le contour de notre objet => noir, sinon on laisse.
...
void main()
{
    ...
    tmpColor = remap(tmpColor);
    tmpColor = contourNoir(tmpColor, normalize(surfel_normal), surfel_to_camera);
