...
vec3 cel(vec3 color)
{
    float levels = 20.0; //On a augmenté les niveaux pour le rendu des égouts car il était trop sombre.
...
vec3 papier(vec3 color, vec3 position)
{
    ...
    float n = fract(sin(dot(position.xy * grain, vec2(12.9898, 78.233))) * 43758.5453);//on utilise la position pour créer du bruit   
